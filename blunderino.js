...
class SpacedRepetition {
  ...
  nextDate (grade) {
    const getNextTime = () => {
      let time = this.config[this.state] || { "seconds": 60 }
...
function updateStatus (prevMove) {
  ...
  if ((prevMove.from === bestMove.from) && (prevMove.to === bestMove.to)) {
    ...
    window.Retool.modelUpdate({ recallSucceeded: true });
    window.Retool.modelUpdate({nextButtonActive: true});
    }
    else if (((prevMove.from === playerMove.from) && (prevMove.to === playerMove.to))) { 
      window.Retool.modelUpdate({ recallSucceeded: false });
      window.Retool.modelUpdate({tryAgainButtonActive: true});
    if (recallSucceeded === null) { 
      window.Retool.triggerQuery('insertRecall') 
    }
  ...
  $pgn.html(game.pgn())
    ...
    function moveBack(nMoves) {
        for (i = 0; i < nMoves; i++) {
            var history = game.history({ verbose: false })
            if (history.length == 0) {
                return
            }
        ...
        window.Retool.modelUpdate({ hasNextMove: true, action: "" });
        updateGame(game.pgn())
    }
    ...
    function moveForward(nMoves) {
        ...
        if (window.backButtonMoves.length == 0) {
            window.Retool.modelUpdate({ hasNextMove: false, action: "" });
        }
        updateGame(game.pgn())
    }
...
function setupBoard() {
    ...
    if (model.board) {
      ...
      bestMove = game.move(model.board.best_move, { sloppy: true })
      recallSucceeded = model.recallSucceeded
      console.log("b", bestMove)
      if (bestMove) {
        game.undo()
      }
      playerMove = game.move(model.board.move, { sloppy: true })
      ...
      bestReply = game.move(model.board.best_reply, { sloppy: true })
    ...
    function updateScore(uciMessage) {
        ...
        if (score[0] == "mate") {
            ...
            window.Retool.modelUpdate({ score: "M" + score[1], scorePos: 100 * scoreSign, action: "updateScore" })
        } else if (score[0] == "cp") {
...
window.Retool.subscribe(function(reToolModel) {
 ...
 if (model.tryAgainPressed === true) {
   ...
   // This will trigger another event, which will be caught by the next condition.
    window.Retool.modelUpdate({ tryAgainPressed: false, tryAgainButtonActive: false, recallSucceeded: null});
